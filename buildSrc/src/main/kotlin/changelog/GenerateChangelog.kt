        ...
        FileWriter(rootChangelogFile).buffered().use { writer ->
            ...
            lines.asReversed()
                .forEach { (message, hashList, preHash) ->
                    if (hashList.size == 1) {
                        writer.appendLine(
                            "- $message ([`${hashList[0]}`](https://github.com/simple-robot/simpler-robot/commit/${hashList[0]}))"
                        )
                    } else {
                        val pre = hashList[0]
                        val post: String = preHash ?: lastTag ?: "HEAD"
                        writer.appendLine(
                            "- $message ([`$pre..${hashList.last()}`](https://github.com/simple-robot/simpler-robot/compare/$pre..$post))"
                        )
                        writer.newLine()
                        ...
                        hashList.forEach { hash ->
                            writer.appendLine(
                                "    - [`$hash`](https://github.com/simple-robot/simpler-robot/commit/$hash)"
                            )
                        }
                        writer.newLine()
